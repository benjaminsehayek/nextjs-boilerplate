...
export async function POST(request: NextRequest) {
  ...
  const { allowed } = checkRateLimit(`gsc-appearances:${user.id}`, 10, 60_000);
  if (!allowed) {
  ...
  // Check cache
  const cacheKey = `appearances:${body.businessId}`;
  const cached = responseCache.get(cacheKey);
  if (cached && Date.now() < cached.expiresAt) {
    return NextResponse.json(
      { searchAppearanceRows: cached.rows },
      { headers: { 'X-Cache': 'HIT' } },
    );
  }
  ...
  // Query Search Analytics with searchAppearance dimension — last 90 days
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[GSC appearances] API error:', res.status, errText);
      return NextResponse.json({ error: `GSC API error ${res.status}` }, { status: res.status });
    }

    const data = await res.json();
    const searchAppearanceRows: unknown[] = data.rows || [];

    // Store in cache
    responseCache.set(cacheKey, { rows: searchAppearanceRows, expiresAt: Date.now() + CACHE_TTL_MS });

    return NextResponse.json({ searchAppearanceRows });
  } catch (e: any) {
    console.error('[GSC appearances] Fetch error:', e);
